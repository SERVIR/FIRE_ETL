...
# KS MOD 2014-06-30     START

# Issue with temp text files just before the line with "arcpy.MakeXYEventLayer_management"
# The temp text files contain a smaller number of headers than columns.
# The Fix
def ks_Fix_Temp_CSV_File(inFileLocation):

    hc_StringToReplace_Old = "FRP"
    hc_StringToReplace_New = "FRP,UnusedZZZ"

    # The file that needs to be modified
    the_FileLocation_In = inFileLocation

    # Make temp file name
    theEXT = the_FileLocation_In[-4:]
    theNewFileNameEnd = "TEMP_" + theEXT

    # Output File (Temp, used for string replace)
    the_FileLocation_Out = the_FileLocation_In.replace(theEXT, theNewFileNameEnd)

    infile = open(the_FileLocation_In)
    outfile = open(the_FileLocation_Out, 'w')

    for line in infile:
        line = line.replace(hc_StringToReplace_Old, hc_StringToReplace_New)
        outfile.write(line)
    infile.close()
    outfile.close()
    ...
    # Remove old file,
    try:
        os.remove(the_FileLocation_In)
    except:
        pass

    # Rename new temp file to the old name
    try:
        os.rename(the_FileLocation_Out, the_FileLocation_In)
    except:
        pass

    # Remove temp file,
    try:
        os.remove(the_FileLocation_Out)
    except:
        pass



# KS MOD 2014-06-30     END
...
class FireExtractValidator(object):
    ...
    def validateExtract(self, ftp_granules_list):
        ...
        self.debug_logger("satellite_ftp_directory ",satellite_ftp_directory) # ks additional log
    ...
    def _getCurrentFireGranules(self, start_datetime, end_datetime, satellite_ftp_directory):
        ...
        self.debug_logger("self.feature_class.fullpath ",self.feature_class.fullpath) # ks additional log
...
class FireTransformer(object):
    ...
    def _createXYEventLayer(self, fire_csv_with_fields, transform_dir):
        ...
        #self.debug_logger("alert 4.B.1")
        arcpy.env.workspace = transform_dir

        #self.debug_logger("alert 4.B.2")
        arcpy.env.overwriteOutput = True

        #self.debug_logger("alert 4.B.3")
        fire_xy_event_layer_name = "fire_xy_layer"

        #self.debug_logger("alert 4.B.4")
        xyc = self.make_xy_event_layer_config

        # KS MOD 2014-06-30     FIX FUNCTION INSERTION POINT
        ks_Fix_Temp_CSV_File(fire_csv_with_fields)

        #self.debug_logger("alert 4.B.5")
        #self.debug_logger("alert 4.B.5: (fire_csv_with_fields) " + str(fire_csv_with_fields))
        #self.debug_logger("alert 4.B.5: (xyc['in_x_field']) " + str(xyc['in_x_field']))
        #self.debug_logger("alert 4.B.5: (xyc['in_y_field']) " + str(xyc['in_y_field']))
        #self.debug_logger("alert 4.B.5: (fire_xy_event_layer_name) " + str(fire_xy_event_layer_name))
        #self.debug_logger("alert 4.B.5: (xyc.get('spatial_reference','')) " + str(xyc.get('spatial_reference','')))
        #self.debug_logger("alert 4.B.5: (xyc.get('in_z_field','')) " + str(xyc.get('in_z_field','')))
        event_layer_result = arcpy.MakeXYEventLayer_management(
            ...
            fire_csv_with_fields, xyc['in_x_field'], xyc['in_y_field'], fire_xy_event_layer_name, xyc.get('spatial_reference','')
        )
        self.debug_logger("MakeXYEventLayer_management result",event_layer_result.status)

        #self.debug_logger("alert 4.B.6")
...
class FireLoader(object):
    ...
    def _updateTableFields(self, fire_meta_dict):
        ...
        #satellite_name = self.satellite_mapping[fire_meta_dict['shortname']]     # SDE Refactor ks 2014-09
